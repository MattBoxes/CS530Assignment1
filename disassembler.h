...
private:
    ...
    void check_Format_1(string op_Name, int row, int currIndex);
    void check_Format_2(string op_Name, int row, int currIndex);
    int check_Format_3_4(string op_Name, OpCode code, int row, int currIndex);

    unsigned int current_Address;
    unsigned int base_Address;
    vector<unsigned int> literal_Addresses;
    int instruction_Check(int row, int currIndex);
